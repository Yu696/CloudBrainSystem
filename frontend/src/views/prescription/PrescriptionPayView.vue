<template>
  <div class="pay-page">
    <div class="page-title">处方药费支付</div>
      ...
      <el-row :gutter="24">
        ...
        <el-col :span="14">
          <div class="cb-card">
            ...
            <div class="cb-card-body">
              <div v-if="prescription" class="detail">
                <div class="detail-row">
                  <span class="detail-label">处方编号</span>
                  <span class="detail-value mono">{{ prescription.prescriptionId }}</span>
                </div>
                <div class="detail-row">
                  <span class="detail-label">处方描述</span>
                  <span class="detail-value">{{ prescription.prescriptionDesc || '未描述' }}</span>
                </div>
                <div class="detail-row">
                  <span class="detail-label">费用</span>
                  <span class="detail-value" style="font-weight: 700; color: #e6a23c">￥{{ prescription.totalAmount }}</span>
                </div>
                <div class="detail-row">
                  <span class="detail-label">状态</span>
                  <el-tag :type="prescription.status === 2 ? 'warning' : 'success'" size="small">
                    {{ prescription.status === 2 ? '待支付' : '已发药' }}
                  </el-tag>
                </div>
                <el-divider />
                <div class="section-subtitle">药品明细</div>
                <div v-for="(item, idx) in prescription.items" :key="idx" class="drug-item-row">
                  <el-tag size="small" type="info">{{ item.drugName }}</el-tag>
                  <span class="drug-detail">{{ item.spec }} | {{ item.dosage }} {{ item.frequency }}</span>
                  <span class="drug-price">×{{ item.quantity }} ￥{{ item.subtotal }}</span>
                </div>
              </div>
              <el-empty v-else-if="!loading" description="处方不存在" :image-size="80" />
            </div>
          </div>
        </el-col>
        ...
        <el-col :span="10">
          <div class="cb-card">
            ...
            <div class="cb-card-body">
              <div v-if="step === 1 && prescription">
                <div class="fee-display">
                  <span class="fee-label">应付金额</span>
                  <div class="fee-amount">
                    <span class="fee-symbol">¥</span>
                    <span class="fee-number">{{ prescription.totalAmount }}</span>
                  </div>
                </div>
                <div class="fee-display" style="margin-top: 8px">
                  <span class="fee-label">钱包余额</span>
                  <span :class="balance < prescription.totalAmount ? 'text-red' : 'text-green'" style="font-size: 16px; font-weight: 600">
                    ￥{{ balance.toFixed(2) }}
                  </span>
                </div>
                <el-divider />
                <el-button type="primary" class="pay-btn" :loading="paying" @click="step = 2" round>
                  确认支付 ¥{{ prescription.totalAmount }}
                </el-button>
              </div>

              <!-- 确认 -->
              <div v-else-if="step === 2 && prescription">
                <div class="fee-display">
              ...
              <div v-else-if="step === 3" class="pay-success">
                ...
                <p class="success-desc">药费已从钱包扣除，请等待发药</p>
...
import { FirstAidKit, Coin, CircleCheckFilled } from '@element-plus/icons-vue'
import { walletBalanceApi, payPrescriptionApi } from '@/api/wallet'
import { getPrescriptionDetailApi } from '@/api/medical'
...
onMounted(async () => {
  if (!prescriptionId) {
    ElMessage.warning('处方不存在')
    router.push('/wallet')
    return
  }
  await Promise.all([loadPrescription(), loadBalance()])
})

async function loadPrescription() {
  loading.value = true
  try {
    const res = await getPrescriptionDetailApi(prescriptionId)
    prescription.value = res.data
    if (prescription.value?.status !== 2) {
      step.value = 3 // 非待支付状态直接显示完成
    }
  } catch {
...
async function handlePay() {
  paying.value = true
  try {
    await payPrescriptionApi(prescriptionId)
...
.section-subtitle { font-size: 14px; font-weight: 600; color: #333; margin-bottom: 8px; }

.drug-item-row {
  display: flex; align-items: center; gap: 8px; padding: 8px 0;
  border-bottom: 1px solid #f0f0f0; font-size: 13px;
}
.drug-item-row:last-child { border-bottom: none; }
.drug-detail { color: #666; flex: 1; }
.drug-price { color: #e6a23c; font-weight: 600; white-space: nowrap; }
